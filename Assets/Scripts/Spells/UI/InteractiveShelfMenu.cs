...
public class InteractiveShelfMenu : MonoBehaviour
{
    ...
    [Header("Cursor camera motion")]
    [SerializeField] private bool cursorLookEnabled = true;
    [SerializeField, Range(0f, 3f)] private float cursorYaw = 1.2f;
    [SerializeField, Range(0f, 2f)] private float cursorPitch = .7f;
    [SerializeField, Range(0f, 1f)] private float focusedCursorLookScale = .35f;
    [SerializeField, Min(.05f)] private float cursorLookSmoothTime = .3f;
    private Vector2 cursorLookAngles;
    private Vector2 cursorLookVelocity;
    ...
    [SerializeField] private MenuInteractionHint shelfHint;
    [SerializeField] private MenuInteractionHint hatHint;
    [SerializeField] private MenuInteractionHint portalHint;
    ...
    private void Update()
    {
        ...
        // Ввод названия и раскрытый список билдов забирают Q/E/R и Escape у полок.
        if (state == ViewState.Shelf && catalog.BuildsHandleInput()) return;
        ...
        {
            // подсветка и нажатие используют одинаковые области и приоритет предметов.
            bool overPortal = !overInterface && portalMenu != null && portalMenu.HitArea.Raycast(ray, out _, 30f);
            bool overHat = !overInterface && !overPortal && customizationDesk != null && customizationDesk.HatHitArea.Raycast(ray, out _, 30f);
            bool overShelf = !overInterface && !overPortal && !overHat && shelfHitArea.Raycast(ray, out _, 30f);
            shelfHint?.Show(overShelf);
            hatHint?.Show(overHat);
            portalHint?.Show(overPortal);
            if (Input.GetMouseButtonDown(0))
            {
                if (overPortal) BeginPortalFlight();
                else if (overHat) BeginDeskFlight();
                else if (overShelf) BeginFlight(true);
            }
            return;
        }
    ...
    // отключённый контроллер не оставляет активные декоративные подсказки.
    private void OnDisable()
    {
        ClearCursorLook();
        if (catalog != null) catalog.SetInteraction(false);
        shelfHint?.Hide();
        hatHint?.Hide();
        portalHint?.Hide();
    }

    // Небольшой поворот от опорного ракурса, без накопления смещения и крена.
    private void UpdateCursorLook()
    {
        Vector2 target = Vector2.zero;
        Rect viewport = menuCamera.pixelRect;
        Vector2 mouse = Input.mousePosition;
        if (cursorLookEnabled && Application.isFocused && Cursor.lockState != CursorLockMode.Locked
            && viewport.width > 0f && viewport.height > 0f && viewport.Contains(mouse))
        {
            Vector2 normalized = new Vector2(
                (mouse.x - viewport.xMin) / viewport.width * 2f - 1f,
                (mouse.y - viewport.yMin) / viewport.height * 2f - 1f);
            float scale = state == ViewState.Overview ? 1f : focusedCursorLookScale;
            target = new Vector2(-normalized.y * cursorPitch, normalized.x * cursorYaw) * scale;
        }

        cursorLookAngles = Vector2.SmoothDamp(cursorLookAngles, target, ref cursorLookVelocity,
            Mathf.Max(.05f, cursorLookSmoothTime), Mathf.Infinity, Time.unscaledDeltaTime);
        Vector3 baseAngles = state == ViewState.Portal ? portalAngles
            : state == ViewState.Desk ? deskAngles
            : state == ViewState.Shelf ? shelfAngles : overviewAngles;
        menuCamera.transform.rotation = Quaternion.Euler(baseAngles)
            * Quaternion.Euler(cursorLookAngles.x, cursorLookAngles.y, 0f);
    }

    // Снимаем только собственный поворот, не затрагивая позицию камеры или текущий полёт.
    private void ClearCursorLook()
    {
        if (menuCamera != null && cursorLookAngles.sqrMagnitude > 0f)
            menuCamera.transform.rotation *= Quaternion.Inverse(
                Quaternion.Euler(cursorLookAngles.x, cursorLookAngles.y, 0f));
        cursorLookAngles = Vector2.zero;
        cursorLookVelocity = Vector2.zero;
    }
    ...
    private void StartFlight(ViewState flightState)
    {
        ...
        // Полёт начинается с видимого поворота; слежение возобновится с нуля после прибытия.
        cursorLookAngles = Vector2.zero;
        cursorLookVelocity = Vector2.zero;
        flightTime = 0;
        state = flightState;
    ...
    private void UpdateFlight()
    {
        ...
        catalog.SetInteraction(toShelf);
    ...
    public bool ApplyBuild(ElementLoadout loadout)
    {
        if (!IsFocused || LocalPlayerSettings.Instance == null ||
            !LocalPlayerSettings.Instance.ApplyLoadout(loadout)) return false;
        RefreshSelection();
        return true;
    }
