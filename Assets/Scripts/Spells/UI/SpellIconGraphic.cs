...
public class SpellIconGraphic : MaskableGraphic
{
    ...
    protected override void OnPopulateMesh(VertexHelper vh)
    {
        ...
        switch(id)
        {
            case "SteamLens": Circle(Vector2.zero,.8f);Circle(Vector2.zero,.55f);Line(V(-.9f,-.8f),V(.9f,.8f));break;
            case "ShardVortex": Circle(Vector2.zero,.8f);Poly(new[]{V(0,.7f),V(-.2f,0),V(0,-.5f),V(.2f,0)});break;
            case "ScaldingMist": Circle(V(-.3f,.15f),.4f);Circle(V(.3f,.15f),.4f);Steam(0);break;
            case "Meteor": Circle(V(-.2f,-.3f),.45f);Line(V(.1f,0),V(.8f,.8f));Line(V(.4f,-.2f),V(.9f,.4f));break;
            case "ThermalSpring": Wave(-.6f);Line(V(0,-.15f),V(0,.8f));Line(V(-.45f,.3f),V(.45f,.3f));break;
            case "BoilingIce": Poly(new[]{V(0,.5f),V(-.5f,-.4f),V(.5f,-.4f)});Steam(-.65f);Steam(.65f);break;
            case "IceBridge": Path(new[]{V(-.8f,-.6f),V(-.4f,.6f),V(.4f,.6f),V(.8f,-.6f),V(-.8f,-.6f)});Line(V(-.65f,-.2f),V(.65f,-.2f));Line(V(-.5f,.2f),V(.5f,.2f));break;
            case "CrystalCrash": Poly(new[]{V(0,.9f),V(-.35f,.3f),V(0,-.5f),V(.35f,.3f)});Wave(-.7f);break;
    ...
    public static Color Tint(Spell value)=>value is AdvancedSpell advanced?advanced.tint:value is TacticalSpell tactical?tactical.tint:value is ElementalSpell elemental?elemental.tint:
