...
public static class CombatFeedbackBuilder
{
 ...
 public static void Apply()
 {
  ...
  var spell=AssetDatabase.LoadAssetAtPath<TacticalSpell>("Assets/Scripts/Spells/Tactical/StoneWall.asset");var data=new SerializedObject(spell);data.FindProperty("description").stringValue="Стена высотой 2,7 м на 5 с. Перекрывает проход и снаряды. Только на свободной земле.";data.ApplyModifiedPropertiesWithoutUndo();
  var go=new GameObject("DamageNumber",typeof(TextMeshPro),typeof(FloatingDamageNumber));
  var label=go.GetComponent<TextMeshPro>();label.text="25";label.font=AssetDatabase.LoadAssetAtPath<TMP_FontAsset>("Assets/TextMesh Pro/Resources/Fonts & Materials/LiberationSans SDF.asset");label.fontSize=6.5f;label.alignment=TextAlignmentOptions.Center;label.rectTransform.sizeDelta=new Vector2(2,1);label.raycastTarget=false;go.GetComponent<FloatingDamageNumber>().label=label;
